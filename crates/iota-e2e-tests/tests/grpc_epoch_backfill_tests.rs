...
/// The running-node background backfill must reject a snapshot whose manifest
/// `chain_id` doesn't match this node's chain BEFORE seeding `epochs_v2`, so a
/// snapshot fetched from the wrong network's bucket cannot corrupt the table.
///
/// One cluster drives both halves with a real `EpochInfo` built from the node's
/// end-of-epoch summaries: a matching chain_id seeds; a mismatching one is
/// rejected and leaves `epochs_v2` untouched.
#[sim_test]
...
/// `epochs_v2_gap` is the startup consistency guard's completeness check: a
/// gRPC node may only start without `state_snapshot_read_config` when it
/// reports no gap. Validates both outcomes against the node's **real executed
/// checkpoints**: a full-history node's freshly-built (init'd) index is
/// complete (no gap → no panic), while an un-initialized empty index past
/// genesis reports a gap (→ would require a snapshot source).
#[sim_test]
