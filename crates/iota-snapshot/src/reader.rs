...
impl StateSnapshotReaderV1 {
    ...
    pub async fn new(
        ...
        let chain_id = manifest
            .chain_id()
            .context("V2 manifest missing chain_id")?;
        ...
        let mut epoch_info_metadata: Option<FileMetadata> = None;
        for file_metadata in manifest.file_metadata() {
            match file_metadata.file_type {
                ...
                FileType::EpochInfo => {
                    if epoch_info_metadata.is_some() {
                        bail!("Manifest contains more than one EPOCH_INFO entry");
                    }
                    epoch_info_metadata = Some(file_metadata.clone());
                }
            }
        }
        let epoch_info_metadata = epoch_info_metadata
            .ok_or_else(|| anyhow::anyhow!("V2 manifest missing required EPOCH_INFO entry"))?;
    ...
    /// fullnode to backfill `epochs_v2` without a full restore; everything is
    /// done in-memory, so no local scratch directory is needed.
    pub async fn read_epoch_info_only(
        ...
        let epoch_info_metadata = manifest
            .file_metadata()
            .iter()
            .find(|metadata| matches!(metadata.file_type, FileType::EpochInfo))
            .ok_or_else(|| anyhow::anyhow!("V2 manifest missing required EPOCH_INFO entry"))?
            .clone();
