...
pub async fn download_formal_snapshot(
    ...
    // Seed the gRPC `epochs_v2` table from the snapshot's EPOCH_INFO so a
    // restored fullnode can serve historical per-epoch metadata without
    // replaying from genesis. Left without its `meta` row (`new_without_init`)
    // so `GrpcIndexesStore::new` initializes in place rather than dropping the
    // restored rows. Seeded unconditionally; harmless if gRPC is never enabled.
    // The block scopes the RocksDB handle to close before the rename below.
    {
        // Seed only on a chain_id match, so a wrong-network snapshot cannot
        // corrupt the gRPC epoch index.
